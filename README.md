...
Change Data Capture (CDC) backup system sử dụng NestJS và PostgreSQL Logical Replication.
...
Trên primary database, chạy script setup:

```bash
psql -h 185.111.159.86 -U mmuser -d mattermost_test -f setup-primary-db.sql
```

### 4. Setup Backup Database (đã hoàn thành)
...
### CDC Management
- `GET /cdc/status` - Trạng thái replication
- `GET /cdc/compare` - So sánh primary vs backup
- `GET /cdc/users/primary` - Users từ primary DB
- `GET /cdc/users/backup` - Users từ backup DB

### User Management  
- `POST /cdc/users` - Tạo user (primary DB)
- `PUT /cdc/users/:id` - Update user (primary DB)
- `DELETE /cdc/users/:id` - Xóa user (primary DB)
